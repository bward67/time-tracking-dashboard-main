...
const displayCards = async (clickedOption) => {
  ...
  data.forEach((item) => {
    ...
    //now we will create a new section which does not currently exist in our HTML
    const section = document.createElement("section");
    //remember this is a .forEach loop so we will get 6

    //then we add a class of card to it
    section.classList.add("card", itemClass);
    //this will be appended to HTML

    const newSection = `<div class="card-${itemClass}">
    <div class="card-bg">
...
clickButtons(btns);
loadData(); //Weekly is displaying when we load or refresh the page
